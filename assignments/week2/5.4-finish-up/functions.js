...
(function() {
	...
	Geo.gps = {
		...
		init: function() {
			debugMessage("Check if GPS is available");
		    ...
		    for (var i = 0; i < locations.length; i++) {
		        ...
		        if(_calculate_distance(locatie, currentPosition)<locations[i][2]){
		            ...
		            if(window.location!=locations[i][1] && localStorage[locations[i][0]]=="false"){
		                ...
		                debugMessage("Player is within a radius of "+ locations[i][2] +" meters from "+locations[i][0]);
		            }
		        }
		    }
		},
	...
	Geo.gmap = {
		...
		generateMap: function(myOptions, canvasID) {
		    debugMessage("Generate Google map and show this in #"+canvasId);
		    map = new google.maps.Map(document.getElementById(canvasId), myOptions);

		    var routeList = [];
		    // Add markers to map depending on the tourtype
		    debugMessage("Drawing locations, tourtype is: "+tourType);
		    for (var i = 0; i < locations.length; i++) {
